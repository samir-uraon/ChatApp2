...
const fetchRequests = async (userId) => {
  try {
    const { data } = await api.get(`/api/friend/requests?userId=${userId}`);
...
const handleAccept = async (id) => {
  try {
    const { data } = await api.post("/api/friend/accept", { requestId: id});
...
setHasNewRequests(false)
  try {
    ...
    await api.post("/api/friend/mark-seen", {
      userId: currentUser._id,
    });
...
const handleAddContact = async () => {
  try {
    setLoading(true);
    const {data}=await api.post("/api/auth/add-contact", { to:email ,from:currentUser._id,senderName:currentUser.name});
...
{formatDateTime(msg.createdAt).slice(8)}                        </div>
                ...
                <>
                  ...
                  <div className="flex gap-3 w-full">
                    <button
                      ...
                      className="flex-1 bg-blue-600 px-1 py-2 sm:py-2 md:py-1 text-sm text-white  rounded-xl hover:cursor-pointer hover:bg-blue-500"
                    >
                      Edit Profile
                    </button>
                    <button
  ...
  >
    Signup
                    ...
                    <button
                      onClick={logout}
                      className="flex-1 bg-red-500 text-white px-1 py-2 sm:py-2 md:py-1 text-sm rounded-xl hover:cursor-pointer hover:bg-red-400"
                    >
                      Logout
                    </button>
                  </div>
